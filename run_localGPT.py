...
def load_model(model_id, model_basename, device_type, LOGGING=logging, model_path=None):
    """
    ...
    """
    logging.info(f"Loading Model on: {device_type}")
    logging.info("This action can take a few minutes!")
    
    if model_path is not None:
        logging.info(f"Using local model file at {model_path}")
    ...
    if model_basename is not None:
        if ".gguf" in model_basename.lower():
            llm = load_quantized_model_gguf_ggml(model_id, model_basename, device_type, LOGGING, model_path=model_path)
            return llm
...
def retrieval_qa_pipline(device_type, use_history, promptTemplate_type="llama", model_path=None, model_id=None, model_basename=None):
    """
    ...
    # )
    llm = Fireworks(
        api_key= "fw_3ZY89WwQMD9EHPCyXRti366F",
...
def main(device_type, show_sources, use_history, model_type, save_qa, model_path):
    """
